...
import com.sk.book.springboot.config.auth.dto.OAuthAttributes;
import com.sk.book.springboot.config.auth.dto.SessionUser;
import com.sk.book.springboot.domain.user.User;
import com.sk.book.springboot.domain.user.UserRepository;
...
public class CustomOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        String userNameAttributeName = userRequest.getClientRegistration().getProviderDetails()
                .getUserInfoEndpoint().getUserNameAttributeName(); //OAuth2 로그인 진행 시 키가 되는 필드 값(Primary 키와 같은 의미)
